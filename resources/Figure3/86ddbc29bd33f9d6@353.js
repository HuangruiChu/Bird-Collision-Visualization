...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["flare-2.json",new URL("./month_files/Feb.json",import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], function(md){return(
md`# Sunburst: Categories of Species in iNaturalist in 2020`
)});
  main.variable(observer("chart")).define("chart", ["partition","data","d3","width","color","arc","format","radius"], function(partition,data,d3,width,color,arc,format,radius)
{
  ...
  const g = svg.append("g")
      .attr("transform", `translate(${width / 2},${width / 2 - 200})`);
  ...
  function clicked(event, p) {
    ...
    centerExit.filter(function(d) {
        console.log(+this.getAttribute("fill-opacity") || (p !== root));
        return +this.getAttribute("fill-opacity") || (p !== root);
      }).transition(t)
        .attr("fill-opacity", d => +(p !== root));
  }
...
d3.scaleOrdinal(d3.quantize(d3.interpolateRgb('#66c2a5', '#80b1d3'), 2).concat(d3.quantize(d3.interpolateCool, data.children.length - 1)))
)});
